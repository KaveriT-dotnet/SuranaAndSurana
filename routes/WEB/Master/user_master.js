...
//*************************** get_status_type START ******************************* */

router.get("/get_status_type", function (req, res) {
    var reqParam=req.body;
    var query = " CALL sp_get_status_type  ";
...
//*************************** insert_status_master START ******************************* */

router.post("/insert_status_master", function (req, res) {
    var reqParam=req.body;
    var query = "SET @status_type=?; SET @status=?; CALL sp_check_status_master (@status_type,@status) ";
    db.query(query,[reqParam.status_type,reqParam.status], function (err, response) {
        console.log("response",response[2][0].total);
        
       var total=response[2][0].total;
       console.log("total",total);
      if (total == 0) {
        if(reqParam.status_id==0){
          var query1="SET @status_type=?; SET @status=?;SET @status_id=?; SET @created_on=?; CALL sp_insert_status_master (@status_type,@status,@created_on,@created_by) ";
          db.query(query1,[reqParam.status_type,reqParam.status,reqParam.status_id,reqParam.created_on,reqParam.created_by], function (err, response) {
            res.send({ status: 1, msg: "Insert Successfully", data: [] });
                })
            }
                else{
            ...
            var query2="SET @status_type=?; SET @status=?; SET @status_id=?; SET @created_on=?; SET @created_by=?; CALL sp_update_status_master (@status_type,@status,@status_id,@created_on,@created_by) ";
            db.query(query2,[reqParam.status_type,reqParam.status,reqParam.status_id,reqParam.created_on,reqParam.created_by], function (err, response) {
